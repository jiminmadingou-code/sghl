...
class _RegisterScreenState extends State<RegisterScreen> with SingleTickerProviderStateMixin {
  ...
  String _formatDate(String input) {
    try {
      ...
      if (p.length == 3) return '${p[2]}-${p[1].padLeft(2,'0')}-${p[0].padLeft(2,'0')}';
    } catch (_) {}
    return input.trim();
  }
  ...
  Future<void> _register() async {
    ...
    try {
      ...
      final response = await dio.post('/auth/register/', data: {
        ...
        'date_naissance': _formatDate(_dateCtrl.text),
        'sexe': _sexe,
      ...
      if (e.type == DioExceptionType.connectionTimeout ||
        ...
        if (body is Map) {
          msg = body['error']?.toString() ?? body['detail']?.toString() ?? 'Erreur $status.';
        } else {
          ...
          child: FadeTransition(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(children: [
                ...
                const Text('Créer votre compte', style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w800)),
                const SizedBox(height: 4),
                Text('Rejoignez votre espace santé', style: TextStyle(color: Colors.white.withValues(alpha: 0.55), fontSize: 13)),
                        ...
                        onTap: () async {
                          final date = await showDatePicker(
                            ...
                            builder: (ctx, child) => Theme(
                              data: ThemeData.light().copyWith(colorScheme: const ColorScheme.light(primary: Color(0xFF6366F1))),
                              child: child!,
                            ),
                          );
                          if (date != null) {
                ...
                Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Icon(Icons.shield_outlined, color: Colors.white.withValues(alpha: 0.4), size: 14),
                  const SizedBox(width: 6),
                  Text('Données chiffrées · Conformité RGPD', style: TextStyle(color: Colors.white.withValues(alpha: 0.4), fontSize: 11)),
                ]),
